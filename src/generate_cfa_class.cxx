...
int main(int argc, char *argv[]){
  try{
    ...
    for(int arg = 1; (arg+1) < argc; arg+=2){
      ...
      TChain *ptr_chain_a(static_cast<TChain*>(in_file.Get("configurableAnalysis/eventA")));
      if(!ptr_chain_a){
	throw std::runtime_error(std::string("Could not find chainA in ")+argv[arg]);
      }

      TChain *ptr_chain_b(static_cast<TChain*>(in_file.Get("configurableAnalysis/eventB")));
      if(!ptr_chain_b){
