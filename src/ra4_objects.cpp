...
double ra4_objects::getDeltaPhiMETN(unsigned int goodJetI, float otherpt, float othereta, bool useArcsin) {
  ...
  double dp = fabs(deltaphi(jets_AK4_phi()->at(goodJetI), mets_phi()->at(0)));
  double dpN = 0.0;
  if(useArcsin) {
...
double ra4_objects::getDeltaPhiMETN_deltaT(unsigned int goodJetI, float otherpt, float othereta) {
  if(goodJetI==999999) return -99;

  double sum = 0;
  for (unsigned int i=0; i< jets_AK4_pt()->size(); i++) {
    if(i==goodJetI) continue;
    if(IsGoodJet(i, otherpt, othereta)){
      double jetres = 0.1;
      sum += pow( jetres*(jets_AK4_px()->at(goodJetI)*jets_AK4_py()->at(i) - jets_AK4_py()->at(goodJetI)*jets_AK4_px()->at(i)), 2);
    }
  }
  double deltaT = sqrt(sum)/jets_AK4_pt()->at(goodJetI);
  return deltaT;
}
double ra4_objects::getMinDeltaPhiMETN(unsigned int maxjets, float mainpt, float maineta,
