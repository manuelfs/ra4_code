...
event_handler_full::event_handler_full(const string &file_name):
  event_handler_base(file_name){
}

void event_handler_full::ReduceTree(int num_entries, const TString &out_file_name, int num_total_entries){
  TFile out_file(out_file_name, "recreate");
  out_file.cd();

  small_tree_full tree;
  ...
  for(int entry = 0; entry < num_entries; ++entry){
    ...
    tree.jets_pt_from_mini().resize(dirty_jets.size());
    ...
    for(size_t idirty = 0; idirty < dirty_jets.size(); ++idirty){
      ...
      tree.jets_pt_from_mini().at(idirty) = jets_pt()->at(ijet);
    ...
    for(size_t ib1 = 0; ib1 < good_jets.size(); ++ib1){
      ...
      vmet.SetPtEtaPhiM(met_corr(), 0., met_phi_corr(), 0.);
...
event_handler_full::~event_handler_full(){
}
...
void event_handler_full::SetMiniIso(small_tree_full &tree, int ilep, int ParticleType){
...
bool event_handler_full::greater_m(const fastjet::PseudoJet &a, const fastjet::PseudoJet &b){
  return a.m() > b.m();
}

vector<fastjet::PseudoJet> event_handler_full::sorted_by_m(vector<fastjet::PseudoJet> pjs){
